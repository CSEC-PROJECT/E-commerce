...
const AdminDashboard = () => {
  ...
  return (
    <div className="flex flex-col lg:flex-row min-h-screen bg-[#FDFDFF] dark:bg-background text-[#7C3AED] dark:text-primary font-sans">
      <Sidebar />
      <main className="flex-1 p-4 md:p-10 overflow-y-auto space-y-6 md:space-y-10 pb-24 lg:pb-10">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="col-span-1 lg:col-span-8 bg-white dark:bg-card p-6 md:p-10 rounded-[2rem] md:rounded-[2.5rem] border border-[#F5F5F7] dark:border-border shadow-sm">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
              <div>
                <h3 className="text-xl font-bold tracking-tight text-[#7C3AED] dark:text-primary">Weekly Sales Performance</h3>
                <p className="text-sm text-[#A78BFA] dark:text-muted-foreground mt-1">Revenue analysis for the past 7 days</p>
              </div>
              <div className="flex bg-[#F4F5F7] dark:bg-muted p-1.5 rounded-2xl w-full sm:w-auto">
                <button onClick={() => handleClick('This Week')} className="flex-1 sm:flex-none px-6 py-2 bg-white dark:bg-card shadow-sm rounded-xl text-xs font-bold text-[#5542F6] cursor-pointer">This week</button>
                <button onClick={() => handleClick('Last Week')} className="flex-1 sm:flex-none px-6 py-2 text-xs text-[#A78BFA] dark:text-muted-foreground font-bold cursor-pointer hover:text-[#5542F6]">Last week</button>
              </div>
            </div>
          ...
          <div className="col-span-1 lg:col-span-4 space-y-6">
            ...
            <div className="bg-white dark:bg-card p-8 rounded-[2rem] border border-[#F5F5F7] dark:border-border shadow-sm">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-[#7C3AED] dark:text-primary">Sales by Region</h3>
        ...
        <section className="bg-white dark:bg-card rounded-[2rem] border border-[#F5F5F7] dark:border-border p-6 md:p-8 shadow-sm">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <h3 className="text-xl font-bold text-[#7C3AED] dark:text-primary">Recent Transaction</h3>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left min-w-[600px]">
              <thead>
                <tr className="text-[#A78BFA] dark:text-muted-foreground text-xs font-bold uppercase tracking-wider border-b border-[#F5F5F7] dark:border-border">
              ...
              <tbody>
                {RECENT_TRANSACTIONS.map((t, idx) => (
                  <tr key={idx} className="group hover:bg-gray-50 dark:bg-muted/50">
                    <td className="py-5 text-[#A78BFA] dark:text-muted-foreground font-medium">{idx + 1}.</td>
                    <td className="py-5 font-bold text-[#7C3AED] dark:text-primary">{t.id}</td>
                    <td className="py-5 text-[#7C3AED] dark:text-primary font-medium">{t.date}</td>
                    <td className={`py-5 font-bold flex items-center gap-2 ${t.color}`}>
                      <span className="w-2 h-2 rounded-full bg-current" /> {t.status}
                    </td>
                    <td className="py-5 font-bold text-[#7C3AED] dark:text-primary">{t.amount}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 pb-8">
          <div className="col-span-1 lg:col-span-8 bg-white dark:bg-card p-6 md:p-8 rounded-[2rem] border border-[#F5F5F7] dark:border-border shadow-sm">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-xl font-bold text-[#7C3AED] dark:text-primary">Best selling product</h3>
              ...
              <div className="overflow-x-auto">
                <table className="w-full min-w-[500px]">
                  <thead>
                      <tr className="bg-[#F4FDF6] text-[10px] text-[#A78BFA] dark:text-muted-foreground uppercase font-bold tracking-widest text-left">
                      ...
                      {BEST_SELLERS.map((p, i) => (
                        <tr key={i} className="hover:bg-gray-50 dark:bg-muted/30">
                            <td className="py-4 flex items-center gap-4 pl-2">
                              ...
                              <span className="font-bold text-sm text-[#7C3AED] dark:text-primary">{p.name}</span>
                            </td>
                            <td className="py-4 text-sm font-bold text-[#7C3AED] dark:text-primary">{p.orders}</td>
                            <td className="py-4">
          ...
          <div className="col-span-1 lg:col-span-4 space-y-6">
              <div className="bg-white dark:bg-card p-8 rounded-[2rem] border border-[#F5F5F7] dark:border-border shadow-sm">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-lg font-bold text-[#7C3AED] dark:text-primary">Top Products</h3>
                ...
                <div className="relative mb-6">
                    ...
                    <input type="text" placeholder="Search" className="w-full bg-[#F4F5F7] dark:bg-muted rounded-xl py-3 pl-11 text-sm focus:outline-none focus:ring-1 focus:ring-[#5542F6]/30" />
                </div>
              ...
              <div className="border-2 border-dashed border-[#E5E7FF] rounded-[2rem] p-8 flex flex-col items-center text-center bg-[#F9FAFF] dark:bg-background">
                <button onClick={() => handleClick('Add Product')} className="w-14 h-14 bg-[#EEF0FF] dark:bg-muted text-[#5542F6] rounded-2xl flex items-center justify-center mb-4 cursor-pointer hover:scale-105 active:scale-95 transition-transform">
                    <Plus size={28} strokeWidth={3} />
                </button>
                <h4 className="font-bold text-lg text-[#7C3AED] dark:text-primary">Add New Product</h4>
                <p className="text-xs text-[#A78BFA] dark:text-muted-foreground mt-2 mb-6 px-4">Select a category below to start listing your next best-seller</p>
                <div className="flex gap-3">
                    <button onClick={() => handleClick('Add Electronics')} className="px-5 py-2 bg-white dark:bg-card border border-[#F0F0F0] dark:border-border rounded-xl text-[10px] font-bold uppercase shadow-sm cursor-pointer hover:border-[#5542F6]">Electronics</button>
                    <button onClick={() => handleClick('Add Fashion')} className="px-5 py-2 bg-white dark:bg-card border border-[#F0F0F0] dark:border-border rounded-xl text-[10px] font-bold uppercase shadow-sm cursor-pointer hover:border-[#5542F6]">Fashion</button>
                </div>
              </div>
          </div>
        </section>
...
const StatCard = ({ title, value, value2, subValue, subDetail, color, isDouble, sub1, sub2, onClick }) => (
  <div className="bg-white dark:bg-card p-6 rounded-[2.5rem] border border-[#F5F5F7] dark:border-border shadow-sm flex flex-col justify-between h-full min-h-[220px]">
    <div>
        <div className="flex justify-between items-center text-[#A78BFA] dark:text-muted-foreground mb-5">
        ...
        {isDouble ? (
            <div className="flex items-start">
                <div className="flex-1">
                    <div className="text-[28px] md:text-[32px] font-black leading-none text-[#7C3AED] dark:text-primary">{value}</div>
                    <div className="text-[9px] text-[#A78BFA] dark:text-muted-foreground uppercase mt-2 font-bold tracking-tight">
            ...
            <>
                <div className="flex items-baseline gap-2 mb-1">
                    <span className="text-[38px] md:text-[44px] font-black leading-none tracking-tight text-[#7C3AED] dark:text-primary">{value}</span>
                    <span className={`text-[10px] font-bold ${color}`}>{subValue}</span>
                </div>
                <p className="text-[9px] text-[#A78BFA] dark:text-muted-foreground mt-1.5 uppercase font-extrabold tracking-widest">{subDetail}</p>
            </>
        )}
    </div>
...
const MiniInfo = ({ label, value, color = "text-[#7C3AED] dark:text-primary" }) => (
    <div>
        <p className="text-[10px] text-[#A78BFA] dark:text-muted-foreground font-bold mb-1 uppercase tracking-wider">{label}</p>
...
const RegionRow = ({ label, percent }) => (
    <div className="mb-5 last:mb-0">
        <div className="flex justify-between text-xs font-bold mb-2">
            <span className="flex items-center gap-2 text-[#7C3AED] dark:text-primary">
                <div className="w-1 h-3 bg-[#5542F6] rounded-full" /> {label}
            </span>
            <span className="text-[#7C3AED] dark:text-primary">{percent}%</span>
        </div>
...
const TopProductItem = ({ name, price, id, img }) => (
    <div className="flex items-center justify-between group cursor-pointer">
        <div className="flex items-center gap-3">
            ...
            <div>
                <p className="text-sm font-bold text-[#7C3AED] dark:text-primary transition-colors group-hover:text-[#5542F6]">{name}</p>
                <p className="text-[10px] text-[#A78BFA] dark:text-muted-foreground font-semibold mt-0.5">Item: {id}</p>
            </div>
        </div>
        <span className="text-sm font-bold text-[#7C3AED] dark:text-primary">{price}</span>
    </div>
);
